...
const programs = [
  ...
  },
  {
    id: 2,
    title: "LE DOSSIER",
    image: "https://res.cloudinary.com/drzoiigek/image/upload/v1774432661/z825xwszdvxlaqtu0nre.png",
    duration: "120 min",
    viewers: "1.8M",
    category: "Basketball",
    live: false,
  ...
  {
    id: 3,
    title: "EDITION SPECIALE",
    image: "https://res.cloudinary.com/drzoiigek/image/upload/v1774432660/nwydr2t5wvhzxjenmgej.jpg",
    duration: "45 min",
    viewers: "950K",
    category: "Tennis",
    live: false,
  ...
  {
    id: 5,
    title: "OEIL DE LA REDACTION",
    image: "https://res.cloudinary.com/drzoiigek/image/upload/v1774432660/wpi9zu3sm30hicwu0fcy.png",
    duration: "120 min",
    viewers: "1.5M",
    category: "Athletics",
    live: false,
...
const catStyle = {
  ...
  Athletics:  "bg-emerald-500/20 text-emerald-400 border-emerald-500/30",
  Boxing:     "bg-violet-500/20 text-violet-400 border-violet-500/30",
};

function ProgramCell({ program, delay }) {
  const [hovered, setHovered] = useState(false);
  const cat = catStyle[program.category] || "bg-white/10 text-white/50 border-white/10";

  return (
    <div
      className={`prog-cell fade-up ${program.span} ${program.minH} relative rounded-2xl overflow-hidden cursor-pointer border bg-zinc-900
        ${program.live
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 z-10">
        ...
        <h3 className={`font-anton ${program.titleSize} leading-tight mb-3 transition-colors duration-200 ${hovered ? "text-red-400" : "text-white"}`}>
          {program.title}
        </h3>
    ...
    <section className="bg-zinc-950 min-h-screen">
      ...
      <div className="max-w-screen-xl mx-auto px-6 md:px-10 py-16 md:py-20">
        ...
        <div className="fade-up flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-10">
          ...
          <div className="flex items-center gap-5">
            <p className="font-outfit text-white/25 text-sm hidden md:block">
              Live sport and premium programming, all in one place.
            </p>
            <a
              href="#"
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
          {programs.map((prog, i) => (
            <ProgramCell key={prog.id} program={prog} delay={i * 70} />
          ))}
        </div>
